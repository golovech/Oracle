...
SELECT *
FROM dba_users; 
-- SQL 오류: ORA-00942: table or view does not exist : 다른곳에서 이미 사용중이다.


-- SCOTT 에서 기본 SELECT문을 익히기 위해 테이블 생성
-- 1) sql plus 실행 -> SYS계정으로 접속.
-- 2) c:\\oraclexe 폴더       scott.sql 파일 검색     파일 위치 열기
SELECT *
FROM tabs; -- SCOTT이 가지고 있는 table 정보 가져오기
-- 오라클은 사용자 계정(scott)을 생성하면, 동일한 이름(scott)으로 DB객체를 생성한다.
--> [스키마(Schema)] 라고 부른다 : 계정에 만들어지는 모든 객체들.
DESC emp; -- 테이블의 사원을? 알아보는 커리를 작성하세요.

-- SAL               NUMBER(7,2)  
-- COMM              NUMBER(7,2) 실수 (소숫점 두 자리)

DESC bonus; -- 테이블의 객체를 알아보는 커리를 작성하세요.
DESCRIBE dept;
-- EMP 테이블에 사원 정보 조회.
-- DQL : SELECT문 ) 테이블, 객체테이블 등으로부터 데이터를 가져오는데 사용.
-- SQL 문은 절(clause)이라 하는 몇 개의 섹션으로 나뉘어지며, SELECT 문에서 사용되는 절은 7개.
-- 처리순서는 아래와 같다.
WITH        -- 1 함께니까 첫번째.
FROM        -- 2 생략불가
WHERE       -- 3
GROUP BY    -- 4
HAVING      -- 5
SELECT      -- 6 생략불가 // DISTANCT, ALL, AS
ORDER BY    -- 7 오도바이는 위험하니까 나중에 타자~ 
...
select *
from dba_tables;
from user_tables; -- 내 소유의 테이블.
from all_tables; -- 내가 접근할 수 있는 모든 테이블.
from tabs;
...
-- 사원들의 속해있는 잡 컬럼만 출력 : 
-- DISTINCT (중복제거)
SELECT DISTINCT job -- 잡을 고르다.
FROM emp; -- 사원에서 가져오다.

SELECT COUNT( DISTINCT job ) -- 집계함수. 몇개인지 셈
FROM emp; 

-- 기본 전부 ALL이 붙어있다.
SELECT ALL empno, ename, job
FROM emp;
